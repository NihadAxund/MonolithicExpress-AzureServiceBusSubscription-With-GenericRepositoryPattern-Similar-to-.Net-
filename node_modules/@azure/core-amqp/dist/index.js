...
function isSystemError(err) {
    if (!coreUtil.isObjectWithProperties(err, ["code", "syscall", "errno"])) {
        return false;
    }
...
function translate(err) {
    ...
    if (coreUtil.isError(err) && rheaPromiseErrors.indexOf(err.name) !== -1) {
    ...
    return coreUtil.isError(err) ? err : new Error(String(err));
}
/**
 * @internal
 */
...
class RequestResponseLink {
    ...
    constructor(session, sender, receiver) {
        ...
        this.sender.on(rheaPromise.SenderEvents.senderError, (context) => {
...
async function retry(config) {
    ...
    if (updatedConfig.retryOptions.mode == undefined) {
        updatedConfig.retryOptions.mode = exports.RetryMode.Fixed;
    }
    const errors = [];
    let result;
    let success = false;
    ...
    for (let i = 1; i <= totalNumberOfAttempts; i++) {
        ...
        try {
            ...
            if (errors[(errors === null || errors === void 0 ? void 0 : errors.length) - 1].retryable && totalNumberOfAttempts > i) {
                ...
                await coreUtil.delay(targetDelayInMs, {
...
const ConnectionConfig = {
    ...
    validate(config, options) {
        ...
        if (coreUtil.isDefined(config.entityPath)) {
...
class CbsClient {
    ...
    async negotiateClaim(audience, token, tokenType, options = {}) {
        ...
        try {
            ...
            logger.warning("[%s] An error occurred while negotiating the cbs claim: %s", this.connection.id, coreUtil.isError(err) ? `${err.name}: ${err.message}` : String(err));
    ...
    async close() {
        try {
            ...
            const msg = `An error occurred while closing the cbs link: ${coreUtil.isError(err) && err.stack ? err.stack : JSON.stringify(err)}.`;
    ...
    remove() {
        try {
            ...
            const msg = `An error occurred while removing the cbs link: ${coreUtil.isError(err) && err.stack ? err.stack : JSON.stringify(err)}.`;
            logger.verbose("[%s] %s", this.connection.id, msg);
            throw new Error(msg);
        }
    }
...
function isSasTokenProvider(thing) {
    return coreUtil.isObjectWithProperties(thing, ["isSasTokenProvider"]) && thing.isSasTokenProvider === true;
}
